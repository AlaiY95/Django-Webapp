...
# to fetch file and year, we use Django forms.
from django import forms
import openpyxl
from supplychain.models import Pollutant, Country, PollutantEntry
from supplychain.helpers import get_headers_and_units, XLHEADERS
...
def airpollution_visual_data1(request):
    ...
    visuals_data['Pollution Levels by Pollutant by Country']['datasets'] += \
        [{'label': c.name, 'backgroundColor': c.color, 'hidden': 'true', 'data': []}
            for c in country_list]

    for pollutant in pollutant_list:
        visuals_data['Pollution Levels by Pollutant by Country']['labels'].append(
            pollutant.name)
        ...
        for i, country in enumerate(country_list):
            ...
            if total is not None and count:
                ...
                visuals_data['Pollution Levels by Pollutant by Country']['datasets'][i +
                                                                                     1]['data'].append(-1)
...
def airpollution_visual_data2(request):
    ...
    for country in all_countries:
        ...
        for year in all_years:
            ...
            if summary_type == 'avg':
                country_tot = PollutantEntry.objects.aggregate(
                    s=Sum('pollution_level', filter=f))['s']
                country_count = PollutantEntry.objects.filter(f).count()
                country_data['data'].append(
                    country_tot / country_count if country_count else 0)
            elif summary_type == 'min':
                country_min = PollutantEntry.objects.aggregate(
                    s=Min('pollution_level', filter=f))['s']
                country_data['data'].append(country_min if country_min else 0)
            else:  # by default -> max
                country_max = PollutantEntry.objects.aggregate(
                    s=Max('pollution_level', filter=f))['s']
...
def temp_country_creator(request):
    ...
    return redirect('supplychain:upload')


def temp_add_colors_to_pollutants(request):
    try:
        pollutants = ['PM2.5', 'PM10', 'NO2', 'O3', 'BaP', 'SO2']
        colors = ['#dc5c5c', '#dc5cdb', '#5c63dc',
                  '#5cdadc', '#66dc5c', '#dcdb5c']
